  ...
  useEffect(() => {
    async function handleCheckoutSuccess(sessionId)
    {
      try {
        ...
        await apios.post('/payments/checkout-success', { sessionId });
        await clearCart();
      }
      catch(err) {
        console.log(error);
  ...
  return (
    <div className='h-screen flex items-center justify-center px-4'>
            ...
            py-2 px-4 rounded-lg transition duration-300 flex items-center justify-center' >
						...
						<Link to={"/"} className='w-full bg-gray-700 hover:bg-gray-600 text-emerald-400
            font-bold py-2 px-4 rounded-lg transition duration-300 flex items-center justify-center' >
							Continue Shopping
